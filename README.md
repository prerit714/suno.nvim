...
**Suno.nvim** is a lightweight Neovim plugin designed to streamline the process
of compiling and running single file programs for supported languages.
Currently, it's optimized for C++ with plans for future language support.
...
## To run locally (if you wish to experiment with this package)

```lua
{
  dir = "~/absolute/path/to/destination/in/your/machine",
  name = "suno",
  lazy = false,
  config = function()
    require("suno").setup()
...
maintained in my spare time, and I may not always have the bandwidth to address
all problems promptly. Additionally, I might not be following all best Neovim
coding practices at the moment, so please pardon that as well. Contributions
are always welcome to help improve the plugin! 😊
